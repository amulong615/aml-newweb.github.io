...
window.addEventListener('DOMContentLoaded', function() {
  // 获取提示框元素
  var cookieCard = document.querySelector('.cookie-card');
  ...
  acceptButton.addEventListener('click', function() {
    cookieCard.style.opacity = '0'; // 设置透明度为0，开始渐出效果
    setTimeout(function() {
      cookieCard.style.display = 'none'; // 隐藏提示框
    }, 500); // 延迟500毫秒后隐藏提示框
  });

  // 在右下角显示提示框并设置渐入效果
  cookieCard.style.position = 'fixed';
  cookieCard.style.bottom = '20px';
  cookieCard.style.right = '20px';
  cookieCard.style.opacity = '0'; // 初始透明度为0
  cookieCard.style.transition = 'opacity 0.5s'; // 设置过渡效果
  setTimeout(function() {
    cookieCard.style.opacity = '1'; // 将透明度设置为1，实现渐入效果
  }, 100); // 延迟100毫秒后开始渐入效果
});